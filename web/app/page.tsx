...
export default function Home() {
  ...
  return <SidebarProvider style={{'--sidebar-width': '226px'} as React.CSSProperties}>
    ...
    <div className="workspace"><header className="topbar"><div className="breadcrumb"><SidebarTrigger className="mobile-trigger"/><span>Workspace</span><ChevronRight size={14}/><strong>Schedule planner</strong></div><span className="snapshot"><span/>2024 historical replay</span></header>
      <main className="main-content"><div className="page-heading"><div><p className="eyebrow">AMHERST MEN’S SOCCER</p><h1>Every opponent is a decision.</h1><p>Build your slate. Understand the upside. See where the risk sits.</p></div><Button className="primary-action" disabled={!config}>Compare schedules <ArrowUpRight size={18}/></Button></div>
        ...
        <aside className="panel forecast"><div className="panel-heading"><div><span className="eyebrow">THE CURRENT PICTURE</span><h2>Room to improve.</h2></div><ChartNoAxesCombined size={22}/></div><div className="forecast-number">{fmt(best?.projection.mean)}<span>projected season NPI</span></div><div className="range-visual"><div className="range-line"/><div className="range-dot"/></div><div className="range-labels"><span>{fmt(best?.projection.p10)}<small>10th percentile</small></span><span>{fmt(best?.projection.p90)}<small>90th percentile</small></span></div><div className="insight"><CircleHelp size={18}/><p><strong>A shortlist, not a certainty.</strong> The leading schedules are close. Compare the ranges and each opponent’s downside before deciding.</p></div></aside></div>

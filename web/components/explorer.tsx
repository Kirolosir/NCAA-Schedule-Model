...
export function Explorer({config,data,compact=false,initial=55}: {config:Config;data:Bootstrap;compact?:boolean;initial?:number}) {
  ...
  return <section className={`panel explorer-panel ${compact?'compact-explorer':''}`}>
    <div className="panel-heading"><div><p className="eyebrow">ONE-GAME CHECK</p><h2>{compact?'What does one opponent add?':'Win reward versus loss risk'}</h2></div><Target size={22}/></div>
    <div className="explorer-body">{!compact&&<div className="explorer-search"><label className="field">Use a real opponent<Combobox items={ratings} value={named} onValueChange={v=>{if(v){setNamed(String(v));setNpi(data.teams.find(t=>t.name===v)!.npi);}}}><ComboboxInput aria-label="Explore a named opponent" placeholder={`Search all ${data.teams.length} team profiles…`}/><ComboboxContent><ComboboxEmpty>No team found.</ComboboxEmpty><ComboboxList>{(name:string)=><ComboboxItem key={name} value={name}>{name}</ComboboxItem>}</ComboboxList></ComboboxContent></Combobox></label></div>}
      <div className="npi-input-line"><div><label htmlFor={compact?'quick-npi':'explore-npi'}>Expected opponent NPI</label><span>{named||'Move the slider to test a hypothetical opponent'}</span></div><input id={compact?'quick-npi':'explore-npi'} aria-label="Expected opponent NPI" type="number" value={npi} min={0} max={100} step={.1} onChange={e=>{const n=Number(e.target.value);if(Number.isFinite(n)&&n>=0&&n<=100){setNpi(n);setNamed(null);}}}/></div>
      <Slider aria-label="Expected opponent NPI slider" value={[Math.min(80,Math.max(30,npi))]} min={30} max={80} step={.1} onValueChange={v=>{setNpi(Array.isArray(v)?v[0]:v);setNamed(null);}}/><div className="slider-labels"><span>30 · Weaker opponent</span><span>80 · Stronger opponent</span></div>
      {(npi<data.rating_range[0]||npi>data.rating_range[1])&&<p className="field-note warning-text">Outside this dataset’s observed range. Arithmetic only; no matching historical profile.</p>}
      {error&&<div role="alert" className="notice error">{error}</div>}
      <div className="matchup-meter"><div><span>Estimated Amherst win chance</span><strong>{fmt(result?.probabilities.win===undefined?undefined:result.probabilities.win*100,0)}%</strong></div><div className="matchup-track"><span style={{width:`${(result?.probabilities.win??0)*100}%`}}/></div><small>{!result?'Loading matchup…':result.probabilities.win>=.7?'Safer matchup':result.probabilities.win>=.45?'Competitive matchup':'Higher-risk matchup'}</small></div>
      <div className={`outcome-tiles ${loading?'updating':''}`} aria-busy={loading}>{(['win','tie','loss'] as Outcome[]).filter(o=>!compact||o!=='tie').map(outcome=><div className={`outcome-tile ${outcome}`} key={outcome}><span>{outcome==='win'?<ArrowUpRight size={16}/>:outcome==='loss'?<ArrowDownRight size={16}/>:<MoveRight size={16}/>}If Amherst {outcome==='win'?'wins':outcome==='tie'?'ties':'loses'}</span><strong>{signed(result?.outcomes[outcome].impact)}</strong><small>change to season NPI</small>{!compact&&<p>New projected NPI <b>{fmt(result?.outcomes[outcome].npi)}</b></p>}</div>)}</div>
    ...
    </div><div className="explorer-caveat"><CircleHelp size={16}/><p><strong>This is a quick one-game estimate.</strong> Use “Compare schedules” for the final answer; that full run recalculates every Division III team’s NPI.</p></div>
  </section>;
}

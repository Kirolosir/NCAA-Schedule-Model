...
export function Settings({config,onChange,onReset,disabled}: {config:Config;onChange:(c:Config)=>void;onReset:()=>void;disabled:boolean}) {
  ...
  return <section className="panel settings-panel"><div className="panel-heading"><h2>Comparison setup</h2><SlidersHorizontal size={18}/></div><div className="settings-body">
    ...
    <label className="field">Comparison speed<Choice label="Comparison speed" value={effort} disabled={disabled} onChange={v=>{const presets:Record<string,[number,number,number]>={quick:[4,4,2],standard:[8,8,4],thorough:[24,64,8]};const p=presets[v];if(p)onChange({...config,analysis_mode:v as Config['analysis_mode'],samples:p[0],validation_samples:p[1],insight_samples:p[2]});}} options={[{value:'quick',label:'Quick · best for trying ideas'},{value:'standard',label:'Standard'},{value:'thorough',label:'Thorough · final shortlist'}]}/></label>
...
export function Conference({config,onChange,disabled}: {config:Config;onChange:(c:Config)=>void;disabled:boolean}) {
  return <details className="panel conference-panel"><summary><span><LockKeyhole size={17}/><strong>Fixed 10-game NESCAC schedule</strong><span className="count-pill">{config.fixed_games.length} games</span></span><ChevronDown size={17}/></summary><div className="conference-list"><p>These opponents cannot change. If a result is already known, enter it here; otherwise leave the game open.</p>{config.fixed_games.map((game,i)=><div className="conference-row" key={game.team}><span>{game.team}</span><Choice label={`${game.team} outcome`} value={game.result||'uncertain'} disabled={disabled} options={[{value:'uncertain',label:'Not played yet'},{value:'win',label:'Win'},{value:'tie',label:'Tie'},{value:'loss',label:'Loss'}]} onChange={v=>onChange({...config,fixed_games:config.fixed_games.map((g,j)=>{if(i!==j)return g;const {result:_old,probabilities:_p,...rest}=g;return {...rest,...(v==='uncertain'?{}:{result:v as Outcome})};})})}/></div>)}</div></details>;
}

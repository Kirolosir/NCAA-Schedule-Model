...
import { CalendarDays, ChevronDown, DollarSign, LockKeyhole, MapPin, Pencil, Pin, Plus, RotateCcw, SlidersHorizontal, X } from 'lucide-react';
...
function CandidateEditor({candidate,status,data,config,onSave,onClose}: {candidate:Candidate;status:Priority|'unavailable';data:Bootstrap;config:Config;onSave:(c:Candidate,status:Priority|'unavailable')=>void;onClose:()=>void}) {
  ...
  const valid=rating!==''&&Number.isFinite(Number(rating))&&Number(rating)>=0&&Number(rating)<=100&&
    travel!==''&&Number.isFinite(Number(travel))&&Number(travel)>=0&&cost!==''&&Number.isFinite(Number(cost))&&Number(cost)>=0&&
    (matchup!=='custom'||(values.every(v=>v!==''&&Number.isFinite(Number(v))&&Number(v)>=0&&Number(v)<=100)&&Math.abs(total-100)<1e-8));
  function save(){
    if(!valid)return;
    onSave({team:candidate.team,recent_npi:Number(rating),matchup,venue,available_dates:dates,
  ...
  return <Dialog open onOpenChange={open=>!open&&onClose()}><DialogContent className="opponent-dialog"><DialogHeader><DialogTitle>{candidate.team}</DialogTitle><DialogDescription>Historical reference: national rank #{team.rank} · {team.record} · NPI {fmt(team.npi)}</DialogDescription></DialogHeader>
    <div className="editor-sections"><section><h3>Matchup outlook</h3><div className="planning-fields"><label className="field">Expected opponent NPI<input type="number" min="0" max="100" step="any" value={rating} onChange={e=>setRating(e.target.value)}/></label><label className="field">Expected matchup<Choice label="Expected matchup" value={matchup} onChange={v=>setMatchup(v as Matchup)} options={[{value:'model',label:'Use historical model'},{value:'favorite',label:'Amherst favored'},{value:'toss_up',label:'Toss-up'},{value:'underdog',label:'Amherst underdog'},{value:'custom',label:'Set exact chances'}]}/></label></div>
      ...
      <section><h3>Practical planning</h3><div className="planning-fields"><label className="field">Scheduling status<Choice label="Scheduling status" value={priority} onChange={v=>setPriority(v as Priority|'unavailable')} options={[{value:'available',label:'Available'},{value:'preferred',label:'Preferred'},{value:'required',label:'Required'},{value:'unavailable',label:'Unavailable'}]}/></label><label className="field">Home or away<Choice label="Home or away" value={venue} onChange={v=>setVenue(v as Venue)} options={[{value:'either',label:'Either / undecided'},{value:'home',label:'Home'},{value:'away',label:'Away'}]}/></label></div>
      <label className="field">Available dates<div className="date-entry"><input aria-label="Add available date" type="date" value={newDate} onChange={e=>setNewDate(e.target.value)}/><Button type="button" variant="outline" disabled={!newDate||dates.includes(newDate)} onClick={()=>{setDates([...dates,newDate].sort());setNewDate('');}}><Plus size={15}/>Add date</Button></div></label>
      <div className="date-chips">{dates.map(day=><span key={day}><CalendarDays size={13}/>{new Date(`${day}T12:00:00`).toLocaleDateString(undefined,{month:'short',day:'numeric',year:'numeric'})}<button aria-label={`Remove ${day}`} onClick={()=>setDates(dates.filter(x=>x!==day))}><X size={12}/></button></span>)}{!dates.length&&<em>No date restriction</em>}</div>
...
export function Settings({config,onChange,onReset,disabled}: {config:Config;onChange:(c:Config)=>void;onReset:()=>void;disabled:boolean}) {
  ...
  return <section className="panel settings-panel"><div className="panel-heading"><h2>Comparison setup</h2><SlidersHorizontal size={18}/></div><div className="settings-body"><div className="inline-setting"><label>Nonconference games to choose</label><Choice label="Nonconference games to choose" value={String(config.open_slots)} disabled={disabled} onChange={v=>onChange({...config,open_slots:Number(v)})} options={[1,2,3,4,5,6,7,8].map(n=>({value:String(n),label:String(n)}))}/></div>
    <div className="planning-fields limit-fields"><label className="field"><span className="field-with-icon"><MapPin size={14}/>Maximum travel miles</span><input type="number" min="0" placeholder="No limit" disabled={disabled} value={config.max_total_travel_miles??''} onChange={e=>optionalNumber('max_total_travel_miles',e.target.value)}/></label><label className="field"><span className="field-with-icon"><DollarSign size={14}/>Maximum total cost</span><input type="number" min="0" placeholder="No limit" disabled={disabled} value={config.max_total_cost??''} onChange={e=>optionalNumber('max_total_cost',e.target.value)}/></label></div>
    <p className="field-note">Leave either practical limit blank to compare without that restriction.</p><label className="field">Comparison speed<Choice label="Comparison speed" value={effort} disabled={disabled} onChange={v=>{const presets:Record<string,[number,number,number,number]>={quick:[4,4,2,1e-6],standard:[8,8,4,1e-7],thorough:[24,64,8,1e-8]};const p=presets[v];if(p)onChange({...config,analysis_mode:v as Config['analysis_mode'],samples:p[0],validation_samples:p[1],insight_samples:p[2],convergence_tolerance:p[3]});}} options={[{value:'quick',label:'Quick · best for trying ideas'},{value:'standard',label:'Standard'},{value:'thorough',label:'Thorough · final shortlist'}]}/></label><p className="field-note">Quick is best while changing opponents. Use Thorough when you are ready to compare the final few options.</p>
...
export function Conference({config,data,onChange,disabled}: {config:Config;data:Bootstrap;onChange:(c:Config)=>void;disabled:boolean}) {
  ...
  function conservative(){const npi=new Map(data.teams.map(team=>[team.name,team.npi]));const ordered=[...config.fixed_games].sort((a,b)=>(npi.get(a.team)??0)-(npi.get(b.team)??0)||a.team.localeCompare(b.team));const result=new Map(ordered.map((game,i)=>[game.team,i<4?'win':i<6?'tie':'loss'] as const));onChange({...config,fixed_games:config.fixed_games.map(game=>({...game,result:result.get(game.team) as Outcome,probabilities:undefined}))});}
